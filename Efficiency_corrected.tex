...
\textit{ To be able to \DIFdelbegin \DIFdel{translate observed signal events into }\DIFdelend \DIFaddbegin \DIFadd{obtain }\DIFaddend a \DIFaddbegin \DIFadd{signal }\DIFaddend branching fraction estimate, the normalisation channel
of \bjpsimumuk is used. Both, for signal and normalisation
channel\DIFaddbegin \DIFadd{, }\DIFaddend the absolute efficiencies, luminosity, the b-quark cross-section \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend fragmentation fractions will
cancel. There are, however, efficiencies that will not cancel and will be necessary \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the final limit setting procedure. In this section, methods of obtaining efficiencies of \DIFaddbegin \DIFadd{the }\DIFaddend selection for \DIFaddbegin \DIFadd{the }\DIFaddend normalisation and signal channel are described. Later, the fitting procedure is outlined.}
...
\DIFdelbegin \DIFdel{Overall }\DIFdelend \DIFaddbegin \DIFadd{The overall }\DIFaddend selection efficiency for signal, $\varepsilon^{s}$, and normalisation, $\varepsilon^{n}$, \DIFdelbegin \DIFdel{includes }\DIFdelend \DIFaddbegin \DIFadd{include }\DIFaddend contributions from the detector acceptance efficiency labelled (GEN); the reconstruction selection efficiency (REC); the offline selection efficiency comprising of trigger (TRG), $J/\psi$ and $\Psi(2S)$ veto (OFF), MVA based selection efficiency (CombiBDT and MisidBDT); fitting region selection efficiency (FR); and the efficiency of the PID requirement (PID). A summary of the method used to extract the signal efficiency is shown in\DIFdelbegin \DIFdel{Table }\DIFdelend ~\autoref{tab:signaleffsummary}. For the normalisation channel, there is no $minq$ region selection and hence \DIFdelbegin \DIFdel{full the }\DIFdelend \DIFaddbegin \DIFadd{the full }\DIFaddend (\textit{generator-level+detector}) simulation is used everywhere apart from \DIFaddbegin \DIFadd{the }\DIFaddend $\varepsilon_{GEN}$, \textit{generator-level}.
...
\bottomrule
 \end{tabular}
 \caption{Method of obtaining efficiencies. Most of these efficiencies are evaluated using simulation\DIFaddbeginFL \DIFaddFL{. However}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{however, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL TRG and PID efficiencies are evaluated using data and/or simulation techniques.}
...
are required for different parts of the selection chain.
 ...
 \begin{itemize} 
	\item Method \Rn{1} - The first two efficiencies, $\varepsilon_{GEN}$, $\varepsilon_{REC}$, for \DIFaddbegin \DIFadd{the }\DIFaddend signal are obtained using privately generated simulation from~\autoref{tab:MCPPass} using
...
In\DIFdelbegin \DIFdel{Equation }\DIFdelend ~\autoref{eq:number}, $\varepsilon_{minq}$ is obtained by dividing the number of generated events in the \textit{generator-level} simulation (mentioned in~\autoref{tab:MCPPass}) with \DIFaddbegin \DIFadd{the }\DIFaddend $minq$ condition imposed, $N_{generated,minq}$, by the total number of generated events, $N_{generated}$. $N_{in\_acc}$ is the number of events in the \textit{generator-level+detector} simulation before reconstruction, $N_{REC,minq}$ is the number of events after reconstruction with the $minq$ condition.
\item Method \Rn{2} - Divide \DIFaddbegin \DIFadd{the }\DIFaddend number of events that passed the selection by the total number of events prior to this particular selection step.
\item Method \Rn{3} - The data-driven approach using \texttt{PIDCalib} package explained in~\autoref{simulationchap} of determining \DIFaddbegin \DIFadd{the }\DIFaddend PID efficiency is used.
 \end{itemize} 

\DIFaddbegin \DIFadd{All of these efficiencies will have a systematic uncertainty associated with them.
These systematic uncertainties are combined into one cumulative systematic which is
taken into account in~}\autoref{systematics} \DIFadd{in the Signal Decay Model section.
}

%DIF > All of these efficiencies will have systematic uncertainty associated with them. These systematic uncertaintied are combined into one systematics which cumulatively taken into account in~\autoref{systematics} in Signal Decay Model section. 
...
Hence resulting values for \DIFaddbegin \DIFadd{the }\DIFaddend relative no fractional corrected mass split efficiency ratios defined in~\autoref{eq:notsplitted} are
...
\label{eq:nofcmeer}
\end{equation}
\DIFdelbegin \DIFdel{Including }\DIFdelend \DIFaddbegin \DIFadd{Upon obtaining the efficiencies with the }\DIFaddend fractional corrected mass split\DIFaddbegin \DIFadd{, which }\DIFaddend defined in~\autoref{eq:splitted}\DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{, the efficiency }\DIFaddend ratios are
...
For charged particles, \DIFaddbegin \DIFadd{the }\DIFaddend detector acceptance efficiency describes the fraction of decays contained in the polar angle region of [10, 400] mrad.  
For \DIFaddbegin \DIFadd{the }\DIFaddend 2012 and 2016 simulation samples, the overall detector acceptance efficiency will be the average \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend two possible magnetic polarity conditions. For 2012 this will be also averaged with two different simulation versions: Pythia 6.4\cite{pythia6} and Pythia 8.1\cite{pythia8}.
...
The reconstruction efficiency is calculated on simulated events which have passed the detector acceptance. For \DIFaddbegin \DIFadd{the }\DIFaddend signal, this efficiency consists of reconstruction and stripping, detailed in~\autoref{tab:stripcutsB}. For \DIFaddbegin \DIFadd{the }\DIFaddend normalisation it consists \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend reconstruction, stripping, \textbf{and on top} the signal stripping is applied. This is done so that \DIFdelbegin \DIFdel{selections in }\DIFdelend \DIFaddbegin \DIFadd{the selections in the }\DIFaddend normalisation and signal \DIFdelbegin \DIFdel{channel }\DIFdelend \DIFaddbegin \DIFadd{channels }\DIFaddend are kept as similar as possible and the fact that the signal selection has tighter cuts as explained in~\autoref{nchannel}. However, it should be noted that \DIFaddbegin \DIFadd{the }\DIFaddend reconstruction efficiency reflects \DIFaddbegin \DIFadd{the }\DIFaddend stripping selection \textbf{without the PID cuts} for both signal and normalisation. This is because \gls{PID} is badly modelled in simulation and hence will be accounted for separately and at the end of the selection chain.

The hierarchy of reconstruction level efficiencies $\varepsilon^{s}_{REC} < \varepsilon^{n}_{REC}$ is also expected as many variables in \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend signal stripping are based on alignment of the mother $B$ with its daughters. For fully reconstructed normalisation channel this is expected to be the case, whereas for not fully reconstructed decays the alignment requirement make the selection tighter. 
...
\label{trigef}
 The trigger efficiency is calculated on top of the (GEN) and (REC) \DIFdelbegin \DIFdel{efficiency}\DIFdelend \DIFaddbegin \DIFadd{efficiencies}\DIFaddend . In order to extract the trigger efficiency, full simulation for both signal and normalisation is used. It should be noted though that at \gls{LHCb}, full simulation is produced based on a certain trigger configuration. The trigger configuration key, TCK, represents a unique code for exact conditions the data have been triggered with at \texttt{L0}, \texttt{HLT1} and \texttt{HLT2}, notably thresholds of certain quantities such as $p$ and $p_{T}$. 

Therefore, if \DIFaddbegin \DIFadd{the }\DIFaddend default TCK for simulation is representative for the whole considered dataset, then the efficiency can be extracted directly from the simulation produced, which is the case for the Run \Rn{1} data.

However in 2016, the trigger thresholds have been changing often resulting in 16 different TCKs with very different $p$ and $p_{T}$ thresholds, see\DIFdelbegin \DIFdel{Table }\DIFdelend ~\autoref{tab:2016MC} for full details. In the second column, \DIFaddbegin \DIFadd{the }\DIFaddend luminosity proportion for 2016 is given. It can be seen that the default simulation in 2016 (corresponding to TCK decimal key 288888335) only represents around 35\% of the data. For this reason, the trigger efficiencies for 2016 data have been obtained by emulation of the trigger on simulation for \texttt{L0} and \texttt{HLT1} level for each individual TCK, creating 16 TCK-based simulations.  This trigger emulation to extract efficiencies was tested with the default trigger configuration (TCK 288888335) to validate the emulation and the correct efficiencies have been recovered. It should be noted that small differences arise from difference between \textit{offline} and \texttt{HLT1} container for \gls{PV}s\DIFdelbegin \DIFdel{which stores the information about }%DIFDELCMD < \gls{minipchi2} %%%
\DIFdel{as the }\DIFdelend \DIFaddbegin \DIFadd{. This is because the }\DIFaddend \gls{PV} finding-algorithm is different \DIFdelbegin \DIFdel{, but these have }\DIFdelend \DIFaddbegin \DIFadd{hence the containers store different }\gls{minipchi2} \DIFadd{but this has }\DIFaddend negligible effect. %\mybox{mentioning this just for reproducibility, but maybe not necessary}.
...
In order to obtain the average efficiency for 2016, the 16 TCK efficiencies are weighted by the proportion of \DIFdelbegin \DIFdel{luminosity corresponding to }\DIFdelend the integrated luminosity \DIFdelbegin \DIFdel{for a given TCK over the full 2016 integrated luminosity}\DIFdelend \DIFaddbegin \DIFadd{taken with the given TCK setting}\DIFaddend .
...
\end{tabular}
\end{center}
\caption{Efficiencies of 2016 trigger emulation on simulation. Depending on \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL TCK \DIFaddbeginFL \DIFaddFL{setting}\DIFaddendFL , the efficiencies vary up 10\% for \texttt{L0} level for \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL signal simulation and up to 5\% for normalisation\DIFdelbeginFL \DIFdelFL{TCK}\DIFdelendFL . This is important as \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL \textit{single event sensitivity} is sensitive to the ratio of these two efficiencies. \DIFdelbeginFL \DIFdelFL{This }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The default }\DIFaddendFL configuration \DIFdelbeginFL \DIFdelFL{is describing }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{describes }\DIFaddendFL correctly \DIFdelbeginFL \DIFdelFL{only }\DIFdelendFL 35\% \DIFaddbeginFL \DIFaddFL{of the }\DIFaddendFL data\DIFdelbeginFL \DIFdelFL{with a high $p_{T}$ threshold}\DIFdelendFL .}
...
For the Misid and Combinatorial BDT selection, the normalisation \bjpsimumuk channel retains more signal than the \Bmumumu channel. This is due to the kaon/muon $p$ and $p_{T}$ \DIFdelbegin \DIFdel{kinematics difference }\DIFdelend \DIFaddbegin \DIFadd{kinematic differences }\DIFaddend as seen in ~\autoref{fig:reason1} and ~\autoref{fig:reason2}, where the kaon track is generally harder than the muon track. \DIFdelbegin \DIFdel{Kaon }\DIFdelend \DIFaddbegin \DIFadd{The kaon }\DIFaddend reconstruction efficiency is worse than \DIFaddbegin \DIFadd{the }\DIFaddend muon reconstruction efficiency because about 11\% of the kaons cannot be reconstructed due to hadronic interactions that occur before the last T station~\cite{LHCb-DP-2013-002}, implying that the $p_{T}$ of the $B$ meson is on average harder for the normalisation channel. As these two quantities are high in \DIFaddbegin \DIFadd{the }\DIFaddend BDT importance ranking\DIFaddbegin \DIFadd{, }\DIFaddend as mentioned in~\autoref{CombiBDTsel}, this makes  the normalisation simulation more efficient. In the Misid BDT selection, again the kinematics of the $B$ meson and the \gls{minipchi2} of the oppositely charged muon to \DIFaddbegin \DIFadd{the mother }\DIFaddend $B$ has a better separation from background than the signal.
...
\caption{(a) Combinatorial BDT response for signal simulation and upper mass sideband as well as for \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL normalisation channel simulation for Stripping 21 and Stripping 26. The most \DIFdelbeginFL \DIFdelFL{discriminative }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{discriminating }\DIFaddendFL variables are (b) $p_{T}$ of the $B$ meson, (c) muon/kaon $p_{T}$ and (d) muon/kaon $p$.}
\label{fig:reason1}
\end{figure}
...
\caption{(a) Misid BDT response for signal simulation and upper mass sideband as well as for \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL normalisation channel simulation for Stripping 21 and Stripping 26. The most \DIFdelbeginFL \DIFdelFL{discriminative }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{discriminating }\DIFaddendFL variables are (b) $p_{T}$ of B, (c) muon IP $\chi^{2}$  and (d) muon/kaon $p$. Misid BDT responses are plotted with combinatorial BDT already applied.}
\label{fig:reason2}
\end{figure}

\subsection{Fitting Range Efficiency (FR)}

As discussed in~\autoref{fittingsel} and in~\autoref{combiback}, the fitting region was chosen in order to avoid modelling a drop in \DIFaddbegin \DIFadd{the }\DIFaddend low corrected mass region for the combinatorial background (exclusion below $4000$ \mevcc) and secondly in order to not include a region where there are very few/no events (exclusion above $7000$ \mevcc) in \textbf{corrected mass}. As seen in~\autoref{tab:effsumarry} and in~\autoref{fig:reasonfitrange} the normalisation channel does not loose many candidates compared to the signal channel.
...
\caption{(a) Visible mass of normalisation and signal simulation. It can be seen that normalisation\DIFaddbeginFL \DIFaddFL{'s }\DIFaddendFL previous preselection has a sharp cut around \DIFdelbeginFL \DIFdelFL{$B$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL visible \DIFaddbeginFL \DIFaddFL{$B$  }\DIFaddendFL mass leading to much higher fitting \DIFdelbeginFL \DIFdelFL{region }\DIFdelendFL efficiency. (b) The corresponding logarithmic version of plot (a).}
...
\label{PIDaff}
As \gls{PID} variables are not correctly modelled in the simulation, mentioned in~\autoref{simulationchap}, a data-driven approach of extracting \DIFaddbegin \DIFadd{the }\DIFaddend PID efficiency is taken. To avoid the introduction of any \DIFdelbegin \DIFdel{any biases in }\DIFdelend \DIFaddbegin \DIFadd{biases in the }\DIFaddend previous steps, especially in \DIFaddbegin \DIFadd{the }\DIFaddend multivariate selection, \DIFaddbegin \DIFadd{the }\DIFaddend PID efficiencies are evaluated at the end of the selection chain with the \texttt{PIDCalib} package data samples.

The PID efficiency is higher for the normalisation channel with all PID requirements given in~\autoref{tab:PIDselectionNorm} compared to \DIFaddbegin \DIFadd{the }\DIFaddend signal provided in~\autoref{tab:PIDselection} due to \DIFaddbegin \DIFadd{the }\DIFaddend weaker PID requirement on the kaon as compared to the muons.
...
Since the PID requirements on the kaon are very loose, there will be a background contribution from $ B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) \pi^{+}$. This contribution is modelled by a double-sided Crystal Ball function to $B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) \pi^{+}$ simulation, where the pion track is given the kaon mass hypothesis. Again, all the parameters apart from mean $\mu^{CB}$ and width $\sigma^{CB}$ are fixed from the fit \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend this simulation. In ~\autoref{fig:FitToPiMuMu}, fits to \DIFaddbegin \DIFadd{the }\DIFaddend \bjpsimumuk simulation and \bjpsimumupi simulation from Stripping 21 are showed using different scales.
For signal, Run \Rn{1} Stripping 21 \DIFdelbegin \DIFdel{simulation }\DIFdelend sample is used and for 2016 the Stripping 26 - TCK 288888335 \DIFdelbegin \DIFdel{simulation }\DIFdelend sample is used. For the \bjpsimumupi \DIFaddbegin \DIFadd{the }\DIFaddend Stripping 21 \DIFdelbegin \DIFdel{simulation }\DIFdelend sample is used for both Run \Rn{1} and 2016.
...
Lastly, \DIFaddbegin \DIFadd{the }\DIFaddend combinatorial background is modelled by an exponential function, where the exponential constant is \DIFdelbegin \DIFdel{let }\DIFdelend \DIFaddbegin \DIFadd{left }\DIFaddend free.% The full fit model, containing description of the individual components as well as their constraints that are propagated to the extended maximum likelihood fit is given in~\autoref{tab:floatingparsummarynorm}. 
...
The $B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}$ signal yield is extracted by performing an unbinned extended maximum likelihood fit with the full fit model to the invariant $\mu^{+} \mu^{-} K^{+}$ distribution in \DIFaddbegin \DIFadd{the range }\DIFaddend $5150\mevcc<M_{B^{+}}<5450\mevcc$. Fits to the $ B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}$ for Run \Rn{1} and 2016 are shown in~\autoref{fig:run1jpsikfitnofcme}. Yields from the fit to \bjpsimumuk are obtained and summarized in~\autoref{tab:normchannelyields}.
...
\end{tabular}
\end{center}
	\caption{ \bjpsimumuk signal yield obtained from fits to \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL $\mu^{+} \mu^{-} K^{+}$ mass spectrum shown in~\autoref{fig:run1jpsikfitnofcme}.}
\label{tab:normchannelyields}
\end{table}

\subsection{Signal Channel Parametrisation}
\label{sigpara}
To fit the signal data, the default fitting strategy is to use a simultaneous unbinned maximum likelihood fit to the $\mu^{+} \mu^{-} \mu^{+}$ corrected mass spectrum \DIFdelbegin \DIFdel{to combined }\DIFdelend \DIFaddbegin \DIFadd{for the  }\DIFaddend Run \Rn{1} and 2016 dataset after the full selection in two bins of fractional corrected mass error, which is denoted as the simultaneous fit. \DIFaddbegin \DIFadd{In this fit, the branching fraction for $\Bmumumu$  is directly fitted as the parameter of interest. }\DIFaddend As a cross-check a non-simultaneous fit, where no splitting in bins of corrected mass is done, is denoted as \DIFaddbegin \DIFadd{the }\DIFaddend non-simultaneous fit. The parametrisation of all the components for the full signal model fit is described below.


\subsubsection{Signal}
The full fit to Run \Rn{1} and 2016 data requires the \DIFdelbegin \DIFdel{signal shape knowledge }\DIFdelend \DIFaddbegin \DIFadd{knowledge of the signal shape }\DIFaddend for the combined dataset. This is obtained from a Run \Rn{1} and 2016 signal simulation \textit{cocktail}. The cocktail is created after full selection by assigning event-by-event weights, \textit{$w^{i}$}, which capture the differences between \DIFaddbegin \DIFadd{the }\DIFaddend Run \Rn{1} and 2016 simulation, that \DIFdelbegin \DIFdel{ware }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend not considered by the full selection.

Firstly, weights that reflect the expected difference due to \DIFaddbegin \DIFadd{the }\DIFaddend increased luminosity are computed. \DIFdelbegin \DIFdel{To obtain these signal weights, the following conditions must be satisfied
}\DIFdelend \DIFaddbegin \DIFadd{Defining $\mathcal{L}^{2012},\mathcal{L}^{2016}$ to be the integrated luminosities and $\sigma^{2012}_{pp \rightarrow b \overline{b}}, \sigma^{2016}_{pp \rightarrow b \overline{b}}$ to be the cross-sections in a given year the following definitions are provided
}\DIFaddend 
...
\DIFaddbegin \DIFadd{The signal weights can be obtained by solving the following system of two equations
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdel{These constraints yield the following value for event-by-event (or rather yearly) weights
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend where $N^{2012},N^{2016}$ \DIFaddbegin \DIFadd{are the }\DIFaddend number of events at \textit{generator level}\DIFdelbegin \DIFdel{, $\mathcal{L}^{2012},\mathcal{L}^{2016}$ are integrated luminosities, and $\sigma^{2012}_{pp \rightarrow b \overline{b}}, \sigma^{2016}_{pp \rightarrow b \overline{b}}$ are cross-sections in a given year}\DIFdelend . $N^{2012},N^{2016}$ number of events at \textit{generator level} is obtained by dividing the number of reconstructed events $N_{REC}$ by the reconstruction efficiency $\varepsilon_{REC}$ (see~\autoref{tab:effsumarry}). Values for these variables are summarized in ~\autoref{tab:MCweightNOFCME}.

\DIFaddbegin \DIFadd{These constraints yield the following value for event-by-event (or rather yearly) weights
}
...
%DIF > $N^{2012},N^{2016}$ number of events at \textit{generator level} is obtained by dividing the number of reconstructed events $N_{REC}$ by the reconstruction efficiency $\varepsilon_{REC}$ (see~\autoref{tab:effsumarry}). Values for these variables are summarized in ~\autoref{tab:MCweightNOFCME}.
...
\bottomrule
\end{tabular}
	\caption{Signal simulation weights used to create \DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL cocktail of mixed Run \Rn{1} (2012) and 2016 \DIFdelbeginFL \DIFdelFL{cocktail}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{events}\DIFaddendFL . The cross-sections listed here are not absolute numbers, but rather relative as only their ratio matters.}
\label{tab:MCweightNOFCME}
\end{table}

Secondly, \DIFaddbegin \DIFadd{an }\DIFaddend event-by-event weight which differs between Run \Rn{1} and 2016 that \DIFdelbegin \DIFdel{need }\DIFdelend \DIFaddbegin \DIFadd{needs }\DIFaddend to be accounted \DIFaddbegin \DIFadd{for  }\DIFaddend is the PID efficiency $\varepsilon_{PID}$ (see~\autoref{tab:effsumarry}), which depends on the kinematics of the final state particles. It is denoted as \DIFdelbegin \DIFdel{$w^{i\varepsilon\{2012,2016\}}_{\varepsilon^{i}_{PID}}$}\DIFdelend \DIFaddbegin \DIFadd{$w^{i\in\{2012,2016\}}_{\varepsilon^{i}_{PID}}$}\DIFaddend . 

The final weight of an event depending on Run \Rn{1} and 2016 is calculated \DIFaddbegin \DIFadd{a
}\DIFaddend 
...
After obtaining \DIFaddbegin \DIFadd{a }\DIFaddend combined Run \Rn{1} and 2016 signal \textit{cocktail}, \DIFaddbegin \DIFadd{a }\DIFaddend fit to this weighted simulation is done with \textbf{the shape} in the corrected mass modelled by a double-sided Crystal Ball function~\DIFdelbegin %DIFDELCMD < \autoref{IP}%%%
...
\caption{Fit to \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL weighted combined signal \textit{cocktail} for (a) NO FCME (b) Low FCME and (c) High FCME split.}
%\vspace*{-1.0cm}
\label{fig:MCSignalFit}
\end{figure}

\textbf{The signal yield}\DIFdelbegin \DIFdel{that is observed in the fit}\DIFdelend , $N^{sig}=N(\Bmumumu)$, is \DIFaddbegin \DIFadd{calculated using several parameters. More specifically, it is }\DIFaddend related to the branching fraction using the normalisation channel in the following way:
\begin{equation}
\begin{split}
...
\label{eq:sigeq}
\end{equation}
where  $\varepsilon^{x}$ is the total selection efficiency \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the channel $x$, $N_{x}$ is \DIFaddbegin \DIFadd{the }\DIFaddend number of $x$ decays, $\mathcal{B}(x)$ is the branching fraction of decay $x$, $R_{\mathrm{FCME}}$ is the relevant efficiency ratio between the two decays detailed in~\autoref{EfficiencySummary} and finally $\alpha$ is the \textit{single event sensitivity}, \DIFaddbegin \DIFadd{a }\DIFaddend variable which describes the sensitivity for the search. Hence, for Run \Rn{1} and 2016 the non-simultaneous fit $N^{sig}$ is a function of six parameters \DIFdelbegin \DIFdel{$N^{sig}(R^{21}_{\mathrm{FCME}},R^{26}_{\mathrm{FCME}},N^{Run\ \Rn{1}}(\bjpsimumuk),N^{2016}(\bjpsimumuk), \mathcal{B}(\bjpsimumuk),\mathcal{B}(\Bmumumu))$}\DIFdelend \DIFaddbegin \DIFadd{$N^{sig}(R^{21}_{\mathrm{FCME}},R^{26}_{\mathrm{FCME}}),N^{Run\ \Rn{1}}(\bjpsimumuk),N^{2016}(\bjpsimumuk), \mathcal{B}(\bjpsimumuk),\mathcal{B}(\Bmumumu))$}\DIFaddend . In the simultaneous case $R^{21}_{\mathrm{FCME}}$,$R^{26}_{\mathrm{FCME}}$, $N^{Run\ \Rn{1}}(\bjpsimumuk)$ and $N^{2016}(\bjpsimumuk) K^{+})$ \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend further split into $\sigma_{\mathrm{lowFCME}}$ and $\sigma_{\mathrm{highFCME}}$ bins, resulting in 10 parameters for $N^{sig}$ in the fit\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, as shown for the non-simultanenous fit parameter summary in~}\autoref{tab:floatingparsummary}\DIFadd{. Again, the parameter of interest in the end is $\mathcal{B}(\Bmumumu)$ which is directly fitted for so N(}\Bmumumu\DIFadd{) is given here just to be able to translate the $\mathcal{B}(\Bmumumu)$ into N(}\Bmumumu\DIFadd{).
}\DIFaddend 
...
%In order to account for the contribution of the partially reconstructed backgrounds in the final signal fit, contaminating yield needs to be estimated and shape needs to be modelled. 
 The simulation sample for \DIFaddbegin \DIFadd{the }\DIFaddend partially reconstructed background that originates through $D^{0}$ decays (~\autoref{partrecobak}) is used for both the yield estimate and shape modelling.
...
All the relevant total selection efficiencies are obtained from the full simulation sample and are shown in~\autoref{tab:prsum}. Due to the usage of a proxy simulation for \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend partially reconstructed decays (using a pion rather than a muon in one case), as discussed in~\autoref{partrecobak}, the trigger efficiency $\varepsilon_{TRG}$ cannot be obtained from simulation\DIFdelbegin \DIFdel{for partially reconstructed decays}\DIFdelend , because the \texttt{HLT2} trigger (see~\autoref{tab:triggersel}) would make \DIFaddbegin \DIFadd{a }\DIFaddend positive decision only either because of finding dimuon pair or two or three-body decays, hence the trigger ratio $\frac{\varepsilon^{\pr}_{TRG}}{\varepsilon^{\bjpsimumuk}_{TRG}}$ 
...
\end{tabular}
\end{center}
\caption{Summary of number of events that \DIFdelbeginFL \DIFdelFL{comes }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{come }\DIFaddendFL from partially reconstructed backgrounds in different bins of FCME, assuming 2012 efficiencies but extrapolating to all samples.}
\label{tab:prsum}
\end{table}

\textbf{The shape} for partially reconstructed backgrounds, $f^{PR}$, is also obtained from the simulation proxy after all the \DIFdelbegin \DIFdel{selection}\DIFdelend \DIFaddbegin \DIFadd{selections}\DIFaddend .
...
\label{misidfitstrat}
The level and the shape of misID background is determined by fitting the misID data samples obtained using the method described in~\autoref{misidprocedure}. A binned $\chi^{2}$ fit is used to extract the shape and yields parameters. The reason for usage of the binned $\chi^{2}$ fit is that the misID samples are \DIFdelbegin \DIFdel{low-statistics }\DIFdelend \DIFaddbegin \DIFadd{little populated }\DIFaddend weighted samples and the shape and yield needs to be propagated to the final data fit while preserving the fit parameter correlations. Since there is a prescale factor of 1\% at stripping level, to obtain the correct yield, the final number needs to be multiplied by 100 to counteract the prescale.

The misID weights obtained from kinematically binned \bjpsikst samples (see ~\autoref{extraction}) have uncertainties associated with them as shown in Figures~\ref{fig:JpsiKnew}\DIFaddbegin \DIFadd{(a)}\DIFaddend ,~\ref{fig:JpsiKaonnew}\DIFaddbegin \DIFadd{(a)}\DIFaddend ,~\ref{fig:JpsiPionnew2016}\DIFaddbegin \DIFadd{(a)}\DIFaddend ,~\ref{fig:JpsiKaonnew2016}\DIFaddbegin \DIFadd{(a)}\DIFaddend . These uncertainties are accounted for in the fit by Gaussian variation of the weights within the uncertainty in a given kinematic bin of $p,\eta$ for each particle species and then folded in to the misID calculation. In this case 100 variations were used. Each variation results in a different template for the misID shape. This misID template is then subsequently binned in 15 bins of corrected mass. From each corrected mass bin, mean $\mu_{var}$ and error $\sigma_{var}$ from gaussianly distributed number of misID events is obtained. 

The total uncertainty due to the weight \DIFaddbegin \DIFadd{$w$}\DIFaddend , $\sigma_{tot}$, for a given bin of corrected mass is calculated using $\sqrt{\sigma_{var}^2 +\sqrt{\sum{w^{2}_{i}}}^{2}}$, where $\sigma_{par}=\sqrt{\sum{w^{2}_{i}}}$ is the associated error per bin and $\sigma_{var}$ is the standard deviation obtained from \DIFaddbegin \DIFadd{the }\DIFaddend variation of misID weights \DIFaddbegin \DIFadd{((see ~}\autoref{extraction}\DIFadd{) in Figures~\ref{fig:JpsiKnew}(a),~\ref{fig:JpsiKaonnew}(a),~\ref{fig:JpsiPionnew2016}(a),~\ref{fig:JpsiKaonnew2016}(a))}\DIFaddend . Finally, \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend binned $\chi^{2}$ fit is made to the misID samples with the total uncertainty. \textbf{The number of misID events}, $N^{MisID}$, for different species-regions after all selections are seen in~\autoref{tab:misidtabcummu}. Hence for the non-simultaneous fit this just \DIFdelbegin \DIFdel{add }\DIFdelend \DIFaddbegin \DIFadd{adds }\DIFaddend 1 parameter and for the simultaneous fit there will be two parameters describing the total \DIFdelbegin \DIFdel{yield of misID }\DIFdelend \DIFaddbegin \DIFadd{misID yield}\DIFaddend .

Also it can be seen in~\autoref{tab:misidtabcummu}, \DIFdelbegin \DIFdel{crossfeedweight }\DIFdelend \DIFaddbegin \DIFadd{the cross-feed weight }\DIFaddend is only considered for kaon-like and pion-like \textit{SS misID} samples. This arises as a consequence of two characteristics of the misID crossfeedweight procedure. First the convergence criteria makes unbalanced samples (one sample very high in misID events and other sample very small in number of misID events) hard to satisfy (case for most of \textit{OS misID} samples). Secondly, proton-like region samples are very sparse and hence it is not necessary to account for crossfeed. %, see Figure ~\ref{fig:ProtonPIDHeatMap}.
...
\includegraphics[width=0.5\linewidth]{./efficiency/signalfits/misiddata/misiddatahighfcmeRun1nolog.pdf}\put(-30,60){(c)}%
	\caption{Binned $\chi^{2}$ fit to the misID templates with no FCME split (b) Low FCME (c) High FCME. In high FCME \DIFdelbeginFL \DIFdelFL{bin}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{case}\DIFaddendFL , the distribution of misID \DIFdelbeginFL \DIFdelFL{is }\DIFdelendFL pollutes \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL signal window more than in the low FCME \DIFdelbeginFL \DIFdelFL{bin}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{case}\DIFaddendFL . Both \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL full weight error $\sigma_{tot}$ and \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL partial weight error $\sigma_{par}$ can be seen.}
%\vspace*{-1.0cm}
\label{fig:MisidFinalFit}
\end{figure}

\subsubsection{Combinatorial Background}
The signal data fit so far includes components for \DIFdelbegin \DIFdel{signal component, }\DIFdelend \DIFaddbegin \DIFadd{the signal, the }\DIFaddend partially reconstructed background\DIFdelbegin \DIFdel{component, and }\DIFdelend \DIFaddbegin \DIFadd{, and the }\DIFaddend misID background component. The only component left to estimate is the combinatorial background. To model the combinatorial component, the exponential function is left floating and fit to data, \DIFdelbegin \DIFdel{which was }\DIFdelend \DIFaddbegin \DIFadd{as }\DIFaddend motivated in~\autoref{combiback}. Hence \textbf{the shape}, $f^{Combi}$, has one(two) parameter(s) for the non-simultaneous (simultaneous) fit, which is the exponential constant. \DIFdelbegin \DIFdel{And }\DIFdelend \textbf{\DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend yield}, $N^{Combi}$, is also parametrised \DIFdelbegin \DIFdel{just with }\DIFdelend \DIFaddbegin \DIFadd{with just }\DIFaddend one (two) parameter(s) for \DIFaddbegin \DIFadd{the }\DIFaddend non-simultaneous (simultaneous) fit.

\DIFaddbegin \DIFadd{Altogether all the parameters for the non-simultanenous fit are summarized in~}\autoref{tab:floatingparsummary}\DIFadd{.
}\DIFaddend %\subsection{Signal Data Fit}
%\label{fitsens}
%
...
\label{eq:fullhypo2}
\end{equation}
where $\sigma$ is the bin of fractional corrected mass. The shared parameters in the two bins of fractional corrected mass error for the simultaneous fit are the branching fractions \DIFdelbegin \DIFdel{$\mathcal{B}(\Bmumumu),\mathcal{B}(\pr),\mathcal{B}(\bjpsimumuk)$}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{B}(\Bmumumu)$, $\mathcal{B}(\pr)$, and $\mathcal{B}(\bjpsimumuk)$}\DIFaddend . 
%non sim 30 param.
%sim 56 param.

\subsubsection{Signal fit model hypotheses}
As the strength of the signal is not a priori known, there are two types of hypotheses that are established: one where there is \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend presence of signal, also known as \DIFaddbegin \DIFadd{the }\DIFaddend signal+background hypothesis \DIFdelbegin \DIFdel{model }\DIFdelend which takes exactly \DIFaddbegin \DIFadd{the }\DIFaddend form of~\autoref{eq:fullhypo} and then a background only hypothesis, where $\mathcal{B}(\Bmumumu)=0 \rightarrow N^{sig}=0$.

\subsubsection{Signal data types}

There are two types of signal datasets as mentioned in~\autoref{Strategy}. Firstly, there is \DIFaddbegin \DIFadd{the }\DIFaddend \textbf{blinded signal data} to which the simultaneous and non-simultaneous fits are performed in order to evaluate the expected sensitivity. Since these datasets are blinded only the mass regions $4000{\mevcc}<M_{\rm{B_{corr}}}<{4500\mevcc}$ and $5500{\mevcc}<M_{\rm{B_{corr}}}<{7000\mevcc}$ are used in the fits. These fits are shown in~\autoref{blindeddatafit}. Secondly fits to \DIFaddbegin \DIFadd{the }\DIFaddend \textbf{full signal data} are performed and these are used in order to set the limit on the $\mathcal{B}(\Bmumumu)$. In these fits the full mass spectrum is used.

\subsection{Signal Fit Systematics}
\label{systematics}
Systematics studies are performed in order to account for possible shortcomings of the methods used in order to measure $\mathcal{B}(\Bmumumu)$. The summary of systematic uncertainties is provided in~\autoref{tab:systematicsummary}. Most of \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend systematic uncertainties directly affect the efficiency ratio between \DIFaddbegin \DIFadd{the }\DIFaddend signal and normalisation \DIFdelbegin \DIFdel{channel}\DIFdelend \DIFaddbegin \DIFadd{channels}\DIFaddend . In that case, fits account for this uncertainty directly by incorporating the uncertainty into \DIFaddbegin \DIFadd{the }\DIFaddend efficiency ratios by adding them in quadrature to the statistical error. For the simultaneous fit these uncertainties are assumed to be $100\%$ correlated between the \DIFdelbegin \DIFdel{bins of FCME }\DIFdelend \DIFaddbegin \DIFadd{FCME bins}\DIFaddend .
...
The largest systematic uncertainty \DIFdelbegin \DIFdel{arise }\DIFdelend \DIFaddbegin \DIFadd{arises }\DIFaddend due to the choice of decay model for the signal channel.
The nominal signal model creates a photon pole, increasing the branching fraction in the low invariant dimuon mass region.
The associated systematic uncertainty is estimated by replacing this decay model with one determined purely by phase-space and by comparing the selection efficiencies, resulting in \DIFaddbegin \DIFadd{a }\DIFaddend $4.6\%$ \DIFaddbegin \DIFadd{total }\DIFaddend systematic uncertainty on the efficiency ratio. \DIFaddbegin \DIFadd{This cumulatively incorporates systematic uncertainties on individual efficiencies $\varepsilon_{GEN}$, $\varepsilon_{REC}$,  $\varepsilon_{TRG}$, $\varepsilon_{OFF}$, $\varepsilon_{BDTs}$ and $\varepsilon_{FR}$.
 }\DIFaddend % In the data fits this uncertainty directly incorporated into efficiency ratios, $R_{\mathrm{FCME}}$, by adding it in quadrature to the statistical error. 
...
\label{eq:tistos}
\end{equation}
In~\autoref{eq:tistos} $N_{TIS\&TOS}$ is the number of \gls{TIS} and \gls{TOS} events passing the trigger requirement and $N_{TIS}$ is the number of \gls{TIS} events \DIFdelbegin \DIFdel{which passed }\DIFdelend \DIFaddbegin \DIFadd{passing }\DIFaddend the trigger requirement.

The efficiency difference of the full trigger chain between \bjpsik decays in \DIFdelbegin \DIFdel{used }\DIFdelend simulation and \textit{Sweighted} data is $3.2\%$. Incorrect emulation of \texttt{HLT2}, which is calculated by comparing \texttt{HLT2} trigger efficiency of the chosen TCK simulation and all the \textit{Sweighted} data, yields $1.5\%$. Altogether the incorrect modelling of the trigger in simulation results in a $3.5\%$ systematic uncertainty.

\subsubsection{Kaon Interaction Probability}
One difference between the signal and the normalisation channels that can have \DIFaddbegin \DIFadd{an }\DIFaddend effect on the efficiencies is that the kaon in the decay \bjpsimumuk can interact with the detector at a probability proportional to the amount of material traversed. The uncertainty on this amount of material leads to a 2\% systematic
uncertainty, \DIFdelbegin \DIFdel{which follows }\DIFdelend \DIFaddbegin \DIFadd{derived following }\DIFaddend the procedure outlined in Ref.~\cite{LHCb-DP-2013-002}.

\subsubsection{Kinematic Reweighting}
Another source of systematic uncertainty is caused by differences in the $B^{+}$ production kinematics in the simulation. Corrected variables are momentum, $p$, transverse momentum $p_{T}$ and vertex $\chi^{2}$ of \DIFaddbegin \DIFadd{the }\DIFaddend $B$. The kinematic weights in a given kinematic bin are calculated using normalised histograms of \bjpsimumuk data, and normalized \bjpsimumuk simulation in a following way
\begin{equation}
w_{(bin)} =  \frac{w_{\bjpsimumuk data,bin}}{w_{\bjpsimumuk simulation,bin}}.
\end{equation}
\DIFdelbegin \DIFdel{Obtained correction weights }\DIFdelend \DIFaddbegin \DIFadd{The correction weights obtained }\DIFaddend are then applied to correct both the \bjpsimumuk and \Bmumumu simulation. The difference between uncorrected and kinematically corrected efficiency ratios is assigned as a systematic uncertainty yielding $1.5\%$ systematic uncertainty on the efficiency ratios.

\subsubsection{Signal Fit Bias and Coverage}
In this section\DIFaddbegin \DIFadd{, the }\DIFaddend systematic uncertainty due to signal bias is evaluated. The pull is calculated using pseudo-experiments where the data is generated for \DIFaddbegin \DIFadd{a }\DIFaddend signal branching fraction $\mathcal{B}= 1.0\times 10^{-8}$ corresponding to $\approx$ 17 signal events for Run \Rn{1} and 2016 data.
These pseudo-datasets are then re-fitted with floating $\mathcal{B}$ and \DIFaddbegin \DIFadd{the }\DIFaddend corresponding number of fitted signal events is obtained. The pull is defined as the difference between number of fitted signal events and the number \DIFdelbegin \DIFdel{events that }\DIFdelend \DIFaddbegin \DIFadd{of events that the }\DIFaddend pseudo-experiment was created with, divided by the error on the number of signal events given by the fit:
\begin{equation}
\frac{N^{orig}_{sig}-N^{fit}_{sig}}{\sigma^{fit}}.
\end{equation}

The pull distributions can be fitted with a gaussian function and the fit bias is calculated as a shift of the mean from 0. The \DIFdelbegin \DIFdel{overcoverage/undecoverage of the fit }\DIFdelend \DIFaddbegin \DIFadd{pull distribution quality of the fits }\DIFaddend is established by shift of the standard deviation of the gaussian function from 1.

For this study 10000 pseudo-experiments were created testing the bias \DIFdelbegin \DIFdel{and coverage }\DIFdelend of both the extended non-simultaneous fit and extended simultaneous fit. The pull distributions for the non-simultaneous fit can be seen in ~\autoref{fig:SignalBias}.
\begin{figure}[H]
\centering
...
As it can be seen, for \DIFaddbegin \DIFadd{the }\DIFaddend non-simultaneous fit \DIFdelbegin \DIFdel{there is overcoverage of 4 $\%$ but }\DIFdelend \DIFaddbegin \DIFadd{the pull distribution follows Gaussian distribution with the value of the standard deviation of $0.961\pm0.007$ which is reasonably good but not perfect. But }\DIFaddend since the statistical error is of the order of 100$\%$ this is \DIFdelbegin \DIFdel{insignificant}\DIFdelend \DIFaddbegin \DIFadd{not so significant}\DIFaddend .
The bias shows 8$\%$ preference for lower signal yields, and hence this will be added as a systematic uncertainty. The same process was repeated for \DIFaddbegin \DIFadd{the }\DIFaddend simultaneous fit and results can be seen in~\autoref{tab:biassum}. 
...
\end{tabular}
\end{center}
\caption{Signal bias estimate from 10 000 pseudo-experiments for both \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL simultaneous and \DIFdelbeginFL \DIFdelFL{not simultaneous }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{non-simultaneous }\DIFaddendFL fit.}
\label{tab:biassum}
\end{table}
...
Most of the parameters in these fits are fixed and if they are not fixed they are constrained with their range allowed to be within $\pm5\sigma$ of the constraint. Error propagation from the parametrisations of different components is dealt with by using two types of constraints: gaussian constraints and multivariate gaussian constraints. The gaussian constraint, \textit{gaussian}, when imposed has \DIFaddbegin \DIFadd{a }\DIFaddend central value of the fitted parameter and as width the error of the fitted parameter. Multivariate gaussian constraint, \textit{mvg\_gaussian}, is \DIFdelbegin \DIFdel{generalisation of }\DIFdelend \DIFaddbegin \DIFadd{a generalisation of the }\DIFaddend gaussian constraint to higher dimensions and is used for misID parametrisation as the correlations between different parameters need to be propagated to the signal fit.
...
\label{sensitivity}
The $CL_{s}$ method\cite{Read:2002hq} is used to produce an expected upper limits from the blinded data fits. This is possible thanks to the fact that this method is based on generation of pseudo-datasets with different signal \DIFdelbegin \DIFdel{hypothesis}\DIFdelend \DIFaddbegin \DIFadd{hypotheses}\DIFaddend . Therefore, it only requires the resulting PDFs of the blinded fits (simultaneous and non-simultaneous) with which pseudo-datasets can be produced.

Systematic uncertainties directly influence the relative efficiencies between signal and normalisation channel. They can therefore be added as gaussian constraints on the relevant efficiency ratio. The expected exclusion limits are then computed and summarized in~\autoref{tab:explimits}. Partial systematics only include systematics related to branching fractions, whereas full systematics include all the effects from~\autoref{tab:systematicsummary}. In \DIFaddbegin \DIFadd{the }\DIFaddend simultaneous fit these effects are assumed to be 100\% correlated between the two bins of fractional corrected mass, but uncorrelated between themselves. As expected, the limit weakens with the addition of systematics. On the other hand there is an \DIFdelbegin \DIFdel{increase }\DIFdelend \DIFaddbegin \DIFadd{improvement }\DIFaddend in expected limit by $20\%$ obtained with the use of the simultaneous fit compared to the non-simultaneous fit with the full systematics included. Addition of 2016 data also improved the expected sensitivity of the search. 
...
In order to perform \DIFaddbegin \DIFadd{the }\DIFaddend fit with signal and background \DIFdelbegin \DIFdel{hypothesis }\DIFdelend \DIFaddbegin \DIFadd{hypotheses }\DIFaddend a good range \DIFdelbegin \DIFdel{of the }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend $\mathcal{B}$ needs to be established in order to aid the fit convergence. This is done the maximizing the log likelihood values for different values of $\mathcal{B}$, also known as profiling \DIFdelbegin \DIFdel{the }\DIFdelend $\mathcal{B}$. The results of the profiling can be seen in~\autoref{fig:prof}. This shows that the minimum is at $\mathcal{B}\sim-2.0\times10^{-8}$, hence the fit prefers a negative value of $\mathcal{B}$. 
...
The simultaneous unbinned extended maximum likelihood fit to all the data with signal+background hypothesis is given in~\autoref{fig:sigfit_unblinded}, converging at value of $\mathcal{B}=-1.8\times10^{-8}$. This hence represents a negative yield fluctuation. The full list of all free and constrained \DIFdelbegin \DIFdel{parameters' }\DIFdelend \DIFaddbegin \DIFadd{parameter }\DIFaddend values obtained in this fit is shown in~\autoref{tab:floatingparsummary_fit}. This fit result then can be translated into yields in all the fitting range and specifically \DIFaddbegin \DIFadd{the }\DIFaddend blinded region by integrating the resulting PDF in a relevant region as shown in~\autoref{tab:yieldiatkos}.
