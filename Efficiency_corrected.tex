...
\DIFaddbegin \DIFadd{All of these efficiencies will have systematic uncertainty associated with them. These systematic uncertaintied are combined into one systematics which cumulatively taken into account in~}\autoref{systematics} \DIFadd{in Signal Decay Model section. 
}
...
\label{eq:nofcmeer}
\end{equation}
\DIFdelbegin \DIFdel{Including }\DIFdelend \DIFaddbegin \DIFadd{Upon obtaining the efficiencies with }\DIFaddend fractional corrected mass split\DIFaddbegin \DIFadd{, which }\DIFaddend defined in~\autoref{eq:splitted}\DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{, the efficiency }\DIFaddend ratios are
...
\label{sigpara}
To fit the signal data, the default fitting strategy is to use a simultaneous unbinned maximum likelihood fit to the $\mu^{+} \mu^{-} \mu^{+}$ corrected mass spectrum \DIFdelbegin \DIFdel{to combined }\DIFdelend \DIFaddbegin \DIFadd{for the  }\DIFaddend Run \Rn{1} and 2016 dataset after the full selection in two bins of fractional corrected mass error, which is denoted as the simultaneous fit. \DIFaddbegin \DIFadd{In this fit, branching fraction for $\Bmumumu$  is directly fitted as the parameter of interest. }\DIFaddend As a cross-check a non-simultaneous fit, where no splitting in bins of corrected mass is done, is denoted as \DIFaddbegin \DIFadd{the }\DIFaddend non-simultaneous fit. The parametrisation of all the components for the full signal model fit is described below.
...
}\DIFdelend \DIFaddbegin \DIFadd{Defining $\mathcal{L}^{2012},\mathcal{L}^{2016}$ to be the integrated luminosities and $\sigma^{2012}_{pp \rightarrow b \overline{b}}, \sigma^{2016}_{pp \rightarrow b \overline{b}}$ to be the cross-sections in a given year the following defitions are provided
}\DIFaddend 
...
\DIFaddbegin \DIFadd{The signal weights can be obtained by solving the following system of the two equations
}\DIFaddend 
...
\textbf{The signal yield}\DIFdelbegin \DIFdel{that is observed in the fit}\DIFdelend , $N^{sig}=N(\Bmumumu)$, \DIFaddbegin \DIFadd{can be obtained by combining the fit result values from other parameters. It }\DIFaddend is related to the branching fraction using the normalisation channel in the following way:
\begin{equation}
\begin{split}
...
\label{misidfitstrat}
The level and the shape of misID background is determined by fitting the misID data samples obtained using the method described in~\autoref{misidprocedure}. A binned $\chi^{2}$ fit is used to extract the shape and yields parameters. The reason for usage of the binned $\chi^{2}$ fit is that the misID samples are \DIFdelbegin \DIFdel{low-statistics }\DIFdelend \DIFaddbegin \DIFadd{little populated }\DIFaddend weighted samples and the shape and yield needs to be propagated to the final data fit while preserving the fit parameter correlations. Since there is a prescale factor of 1\% at stripping level, to obtain the correct yield, the final number \DIFdelbegin \DIFdel{needs }\DIFdelend \DIFaddbegin \DIFadd{need }\DIFaddend to be multiplied by 100 to counteract the prescale.
...
The pull distributions can be fitted with a gaussian function and the fit bias is calculated as a shift of the mean from 0. The \DIFdelbegin \DIFdel{overcoverage/undecoverage }\DIFdelend \DIFaddbegin \DIFadd{coverage }\DIFaddend of the fit is established by shift of the standard deviation of the gaussian function from 1.
...
As it can be seen, for \DIFaddbegin \DIFadd{the }\DIFaddend non-simultaneous fit \DIFdelbegin \DIFdel{there is overcoverage of 4 $\%$ but }\DIFdelend \DIFaddbegin \DIFadd{the value of the standard deviation is $0.961\pm0.007$ which means that the coverage is reasonably good but not perfect. But }\DIFaddend since the statistical error is of the order of 100$\%$ this is \DIFdelbegin \DIFdel{insignificant}\DIFdelend \DIFaddbegin \DIFadd{not so significant}\DIFaddend .
