...
The \Gls{LHC} is a complex machine which accelerates beams of protons in opposite directions in a $\sim$ 27km long circular tunnel. It is located
...
%	\includegraphics[width = 0.5\textwidth]{figs/detector/intlumi.png}\put(-15,100){(a)}
	...
	\caption{Development of the instantaneous luminosity for \Gls{ATLAS}, \Gls{CMS} and \Gls{LHCb} during a random representative LHC fill. After ramping to the desired value of $4\times10^{32}\mathrm{cm^{-2}s^{-1}}$
...
\noindent In the \Bmumumu analysis the variables \texttt{IsMuon} and \texttt{DLLmu} are used to identify muons. In addition, other variables that are used for muon identification in the search for \Bmumumu, are described in~\autoref{otherpid}. The use of several variables for muon identification is done as they are mostly complimentary, exploiting different information from different parts of the detector. 
