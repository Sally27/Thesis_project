...
The summary of expected yield is summarized in~\autoref{tab:prsum}. For the non-simultaneous final fit there are hence 5 parameters $N^{PR}(R^{21}_{FCME}(\pr),N^{Run\ \Rn{1}}(\bjpsimumuk),N^{Run\ \Rn{2}}(\bjpsimumuk), \mathcal{B}(\bjpsimumuk),\mathcal{B}(\pr))$ and for simultaneous data fit with FCME splitting 8 parameters.  The total yield expected for this type of background is very low compared to other expected backgrounds.
...
%	& $N^{sig}$& Free & depend $\mathcal{B}$ & depend $\mathcal{B}$  & depend $\mathcal{B}$ & - & ~\autoref{eq:sigeq} \\ \midrule
	...
	&	$ R^{21}_{FCME}(\Bmumumu) $ & Constrained & & & & gaussian & ~\autoref{eq:nofcmeer}\\%/~\autoref{eq:allfcmeer} \\  
	&	$ R^{26}_{FCME}(\Bmumumu) $ & Constrained & & & & gaussian & ~\autoref{eq:nofcmeer}\\%/~\autoref{eq:allfcmeer}  \\  
...
%	&	$N^{PR}$ & Constrained & 58.6$\pm$5.56& 32$\pm$3.05 & 25.3$\pm$2.49 &  gaussian &  ~\autoref{tab:prsum} \\ \midrule
	...
	&	Others & Fixed & \multicolumn{5}{c}{fixed to values seen in~\autoref{fig:MisidFinalFit}}\\ \midrule
	&	\multicolumn{7}{c}{PartReco Shape Parameters (sum of two Crystal Ball functions)} \\ \midrule
	&	All & Fixed & \multicolumn{5}{c}{fixed to values seen in~\autoref{fig:PRFit}} \\ \midrule
	&	\multicolumn{7}{c}{Signal Shape Parameters (Double-sided Crystal Ball function)} \\ \midrule
	&	All & Fixed & \multicolumn{5}{c}{fixed to values seen in~\autoref{fig:MCSignalFit}} \\ \midrule
...
\subsection{Signal fit systematic effects}
Systematics studies are performed in order to account for possible shortcomings of the methods used. The summary of systematic uncertainties is summarized in \autoref{tab:systematicsummary}. The aim of this search is to either measure or set the limit on the $\mathcal{B}(\Bmumumu)$. In the~\autoref{tab:systematicsummary}, the first two systematics are directly related to the knowledge of the normalisation branching fraction.
...
The associated systematic uncertainty is estimated by replacing this decay model with one determined purely by phase-space, yielding a $4.6\%$ systematic uncertainty on the efficiency ratio. In the data fits this uncertainty directly incorporated into efficiency ratios, $R_{FCME}$, by adding it in quadrature to the statistical error. 

\subsubsection{Trigger}
The second most important systematic effect related to the efficiency ratio is due to incorrect modelling of the trigger in simulation.
This uncertainty is assigned by comparing the difference between the trigger efficiency of \bjpsik decays in simulation and data,
yielding a $3.5\%$ systematic uncertainty.

The trigger efficiency is calculated using \texttt{TISTOS method}, whereby the events that are \gls{TIS} and \gls{TOS} are assumed to be independent. Hence, the efficiency
for selection of \gls{TOS} candidates hence can be calculated by

\begin{equation}
\varepsilon_{TOS} = \frac{N_{TIS\&TOS}}{N_{TIS}}.
\label{eq:tistos}
\end{equation}
In~\autoref{eq:tistos} $N_{TIS\&TOS}$ is the number of $TIS$ and $TOS$ events passing the trigger requirement and $N_{TIS}$ is the number of \gls{TIS} events which passed the trigger requirement.


One difference between the signal and the normalisation channels is that the kaon in the decay \bjpsik can interact with the detector
at a probability proportional to the amount of material traversed. The uncertainty on this amount of material leads to a 2\% systematic
...
Correction weights are calculated using \bjpsik and the difference in the relative efficiency between the signal and the
normalisation channels is assigned as a systematic uncertainty.

Other smaller systematic uncertainties are assigned due to a small fit bias due to the low amount of data available,
the finite size of the simulation and the branching fraction of the decay $\jpsi \to \mumu$.
...
The nominal signal model creates a photon pole, increasing the branching fraction in the low invariant dimuon mass region.
The associated systematic uncertainty is estimated
by replacing this decay model with one determined purely by phase-space, yielding a $4.5\%$ systematic uncertainty.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\linewidth]{./efficiency/unblindedfits/MyOwnProfileLL.pdf}%
