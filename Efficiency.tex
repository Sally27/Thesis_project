...
\newpage
\section{Mass Fits}
	In this section, firstly parametrisation together with the fit to normalisation channel is described in~\autoref{normfit}. This is followed by the signal fit parametrisation outlined in~\autoref{Fit} resulting in blinded and non-blinded data fits described in~\autoref{fitsens}.
...
\textbf{Signal fit model}
The full signal fit model described by probability density function (PDF) consists of all the mentioned components. There are two types of fit models that are used to fit data: non-simultaneous and simultaneous fit model, where the latter is the one which is used for limit setting. Moreover there are two types of hypotheses that needs to be established: one where there is a presence of signal, also known as signal+background hypothesis model, $f(s+b)$ defined as
\begin{equation}
f(s+b)= N^{sig}\times f^{sig} + N^{misID}f^{misID} + N^{PartReco}f^{PartReco} + N^{combi} \times f^{combi}
	\label{eq:fullhypo}
\end{equation} 
and background only hypothesis, which is the same as $f(s+b)$ but with $\mathcal{B}(\Bmumumu)=0$. In~\autoref{eq:fullhypo}, $N^{sig}=N(\Bmumumu)$ in the observed number of signal events which are related to the branching fraction using normalisation channel in a following way:
\begin{equation}
{\mathcal{B}(\Bmumumu)}=\alpha \times N(\Bmumumu)
\end{equation}
\begin{equation}
	=\underbrace{\frac{\mathcal{B}(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}) \times \epsilon_{B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}}}{N(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}) \times \epsilon_{B^{+} \rightarrow \mu^{+} \mu^{-} \mu^{+} \nu}}}_{\text{$\alpha$}} \times N(B^{+} \rightarrow \mu^{+} \mu^{-} \mu^{+} \nu),
\end{equation}
\begin{equation}
	=\underbrace{\frac{\mathcal{B}(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+})}{N(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}) \times  R_{FCME}}}_{\text{$\alpha$}} \times N(B^{+} \rightarrow \mu^{+} \mu^{-} \mu^{+} \nu),
\end{equation}
where $N_{(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+})}$ and $N_{B^{+} \rightarrow \mu^{+} \mu^{-} \mu^{+} \nu}$, $\alpha$ is known as $\textit{single event sensitivity}$, $R_{FCME}$ is the relevant efficiency ratio detailed in~\autoref{EfficiencySummary}. Hence for Run \Rn{1} and Run \Rn{2} altogether for non-simultaneous fit $N^{sig}$ is a function of six parameters $N^{sig}(R^{21}_{FCME},R^{26}_{FCME},N^{Run \Rn{1}}(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}),N^{Run \Rn{2}}(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}), \mathcal{B}(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}),\mathcal{B}(\Bmumumu))$. In  simultaneous case $R^{21}_{FCME}$,$R^{26}_{FCME}$, $N^{Run \Rn{1}}(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+})$ and $N^{Run \Rn{2}}(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+})$ is further split into $\sigma_{lowFCME}$ and $\sigma_{highFCME}$ bins.

$N^{PartReco}$ is obtained in the same way.


\textbf{Blinded signal data fit}.
In order to be able to get the expected sensitivity for this search a simultaneous unbinned maximum likelihood extended fit to the blinded data of corrected mass after the full selection in two bins of FCME is performed. As a crosscheck, also fit with no FCME split is done. The summary of all the components adding to the total probability density function (PDF) for the signal data fit, their modelling and constraints are shown in~\autoref{tab:floatingparsummary}. There are two PDFs that are used for sensitivity studies: signal+background PDF $f(s+b)$ defined as 
\begin{equation}
f(s+b)= N^{sig}\times f^{sig} + N^{misID}f^{misID} + N^{PartReco}f^{PartReco} + N^{combi} \times f^{combi}
\end{equation}
and background only PDF, which is the same as $f(s+b)$ but with $\mathcal{B}(\Bmumumu)=0$.

Most of the parameters of the fit are fixed and if they are not fixed their range is allowed to be within $\pm5\sigma$. Error propagation from the parametrisations of different components is dealt with by using two types of constraints: gaussian constraints and multivariate gaussian constraints. The gaussian constraint, \textit{gaussian}, when imposed has central value of the fitted parameter and as width the error of the fitted parameter. Multivariate gaussian constraint, \textit{mvg\_gaussian}, is generalisation of gaussian constraint to higher dimensions and is used for misID parametrisation as the correlations between parameters' errors need to be propagated to the signal fit.

The fit to blinded data after all selection and with modelling of its components is shown in~\autoref{fig:sigfitpretty} for all categories of FCME. As the signal region is blinded, the fit model for now is reduced by the signal shape, which leaves background-only model. The total  Number of expected background events, $N_b$, is then obtained by integrating the total background PDF (misid PDF + partreco PDF + combinatorial PDF) in the signal region. Number of signal events is obtained from the fit and can be translated into branching fraction $\alpha=\frac{\mathcal{B}(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}) \times \epsilon_{B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}}}{N_{(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+})} \times \epsilon_{B^{+} \rightarrow \mu^{+} \mu^{-} \mu^{+} \nu}}$

\begin{equation}
\mathcal{B}(B^{+} \rightarrow \mu^{+} \mu^{-} \mu^{+} \nu) = \alpha \times N_{B^{+} \rightarrow \mu^{+} \mu^{-} \mu^{+} \nu}
\end{equation}
\begin{equation}
=\underbrace{\frac{\mathcal{B}(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}) \times \epsilon_{B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+}}}{N_{(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+})} \times \epsilon_{B^{+} \rightarrow \mu^{+} \mu^{-} \mu^{+} \nu}}}_{\text{$\alpha$}} \times N_{B^{+} \rightarrow \mu^{+} \mu^{-} \mu^{+} \nu},
 \end{equation}
where $N_{(B^{+} \rightarrow (J/\psi \rightarrow \mu^{+} \mu^{-}) K^{+})}$ and $N_{B^{+} \rightarrow \mu^{+} \mu^{-} \mu^{+} \nu}$
