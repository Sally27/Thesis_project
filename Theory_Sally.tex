...
where the famous maximal (V-A) structure of weak charged current comes from. The first part of $\mathcal{L}_{NC}$ represents known electromagnetic interaction $e=gsin\theta_{W}$. 
...
The leptonic term for one family of leptons is
...
and defining $\phi_{c}=\binom{\phi^{0*}}{\phi^{-}}$ the quark term,
...
where h.c stands for Hermitian conjugate, $i,j$ are the generations over which it is summed, $y^{q}$ are $3\times3$ matrices defining strenghts between generations with each $y_{q}$. After spontaneous symmetry breaking (~\autoref{eq:ref}), the leptonic interaction term is
...
making the mass term to be $m_{l}=\frac{g_{l}v}{\sqrt{2}}$. In a similar way for quarks,
...
Changing into the mass eigenstate basis, however, does not affect most of the Lagrangian. More specifically, there will be no change to the $L_{NC}$ in~\autoref{eq:NC} when expressed in mass eigenstates (hence at tree-level there are no flavour changing neutral-currents in the SM), however, charged current $L_{CC}$ in~\autoref{eq:LC} is affected. Due to the diagonalisation of the mass matrices $L_{CC}$ now includes non diagonal couplings for the current as seen in the $L_{CC}$ for all three fermion generations is:
...
%\end{equation}
where  $V_{ij}=V_{CKM}= U_{uL} U_{dL}^\dag$ is the Cabibbo-Kobayashi-Maskawa (CKM) mixing matrix. In the SM there is an assumption that $V^{\dagger}_{CKM}V_{CKM}=1$, or that CKM mixing matrix is unitary.



\section{Quark Mixing Matrix}
As mentioned above, from transformation of mass matrices via two unitary matrices one obtains the CKM matrix which exhibits a strong hierarchy in the size of the matrix elements of the quark mixing matrix, which is still an unsovled puzzle. From the previous discussion the quark mixing matrix is a $3 \times 3$ complex unitary matrix yielding 18 parameters to start with. Unitarity of the CKM matrix implies that matrix elements are orthonormal, reducing the count of free parameters to 9. Further, 5 out of 6 quark phases can be absorbed into the redefinition of the quark field, cutting the number of parameters down to 4 parameters, three quark mixing angles and one CP violating phase. There are many different parametrisations which are all mathematically equivalent to the CKM matrix, but the standard parametrisation of the CKM matrix for flavour mixing is the following:
...
A geometrical interpretation of $CP$ violation is offered by the concept of unitarity triangles. Unitarity of the CKM matrix can be summarized by two sets of orthogonality relations:
...
V_{cd}V^*_{cb}
\end{equation}
and take relation of $\bar{\rho}$ and $\bar{\eta}$ to ${\rho}$ and ${\eta}$ 
\begin{equation}
...
it can be pictorially represented in the $\bar{\rho}$ and $\bar{\eta}$ plane as in~\autoref{fig:unitr}.
\begin{figure}[h]
\centering
...
The area of these triangles are half of the Jarlskog invariant \textit{J}, a quantifier of CP violation, which is defined as $Im[V_{ij}V_{kl}V^*_{il}V^*_{kj}]$ \cite{Jarlskog:1985ht}. It is interesting to notice that the SM with its parameters may or may not violate CP. Only after measuring the parameter it is possible to determine the CP non-conservation. \textit{J} vanishes only if mixing angle $\theta_{ij} = 0 , \pi/2$; $\delta =  0 , \pi$; So measurements of \textit{J} allows to verify that the CKM matrix is complex and hence different mixing for quarks and anti-quarks is obtained, providing theoretical grounding for CP violation, although SM CP violation is not big enough to explain the matter dominated universe.

The CKM matrix elements which comprises of magnitudes and phases can be determined in different ways but the most precise option employs a global fit to
all available measurements (as seen in~\autoref{fig:unifit}) while imposing the SM constraints such as the unitarity of CKM matrix. Hence, the most precise measurement of the CKM matrix magnitudes \mybox{export 2018 citation PDG} to-date is 
...
with non-zero Jarlskog invariant $J=(3.18\pm0.15)\times 10^{-5}$. Higlighted is the result for magnitude of the $V_{ub}$ matrix element $|V_{ub}|$ which is the element with highest fractional uncertainty on its value. Therefore precise measurement of this element is very important. Moreover, in~\autoref{fig:unifit}, it can be noticed that the measurement of $|V_{ub}|$ together with $sin(2\beta)$ measurement constrain apex of the triangle and indirectly the unitariy of the CKM matrix, which is one of the assumptions of the SM.


\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{theory/rhoeta_large_lol.eps}
\caption{Different experimental measurements that constrain the CKM matrix elements together with the fit result.}
\label{fig:unifit}
\end{figure}
