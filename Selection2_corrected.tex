...
\textit{In this chapter, the selection for the search \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend \Bmumumu decays at LHCb is presented. This search is challenging because of the rareness of its occurrence as well as the different backgrounds that can mimic its signature in the detector. Moreover, \DIFaddbegin \DIFadd{the }\DIFaddend presence of \DIFdelbegin \DIFdel{of }\DIFdelend the invisible neutrino in the decay induces uncertainties into the reconstruction. This chapter concentrates on \DIFaddbegin \DIFadd{the }\DIFaddend data selection which reduces background contamination. \DIFdelbegin \DIFdel{Normalisation }\DIFdelend \DIFaddbegin \DIFadd{The normalisation }\DIFaddend channel along with its selection is also discussed. In the end, a method to improve \DIFaddbegin \DIFadd{the }\DIFaddend sensitivity is introduced.}
...
Throughout the analysis there was a blinding procedure put in place for this search in order not to bias the result. The \DIFaddbegin \DIFadd{corrected mass of $B^{\pm}$ $M_{\rm{B_{corr}}}$, which is detailed in~}\autoref{eq:corrm}\DIFadd{, is the fit variable for this search. The }\DIFaddend signal region $4500{\mevcc}<M_{\rm{B_{corr}}}<{5500\mevcc}$ was blinded until the full strategy and sensitivity was evaluated. The signal dataset corresponding to this selection is known as \textbf{blinded signal dataset}. After unblinding, data with the full mass spectrum was obtained and is known as \textbf{full signal dataset}.%  Hence in all blinded fits only data in regions $4000{\mevcc}<M_{\rm{B_{corr}}}<{4500\mevcc}$ and $5500{\mevcc}<M_{\rm{B_{corr}}}<{7000\mevcc}$ are used.



In~\autoref{fitsens}, two fitting strategies for the signal data are described. One is more sensitive than \DIFdelbegin \DIFdel{another}\DIFdelend \DIFaddbegin \DIFadd{the other}\DIFaddend , where the fitting strategy that provides the best sensitivity makes use of simultaneous \DIFdelbegin \DIFdel{fit }\DIFdelend \DIFaddbegin \DIFadd{fits }\DIFaddend to two bins of resolution, increasing signal separation from background. More information about this split can be found in~\autoref{split}. Both fitting strategies were applied on blinded signal datasets in order to compute the expected exclusion limit on the branching fraction, shown in~\autoref{sensitivity}. \DIFaddbegin \DIFadd{In both cases, the branching fraction of $\Bmumumu$ decay, the quantity of interest, is directly fitted for. }\DIFaddend Systematic uncertainties that \DIFdelbegin \DIFdel{are affecting }\DIFdelend \DIFaddbegin \DIFadd{affect }\DIFaddend this measurement are presented in~\autoref{systematics}.
...
\label{eq:corrm}        
\end{equation}	
where the $M^{2}$ is the invariant visible mass squared and $p^{2}_{T}$ is the missing momentum squared transverse to the direction of the $B^{+}$ meson flight. \DIFaddbegin \DIFadd{It is defined in~}\autoref{eq:ptmis}\DIFadd{. }\DIFaddend The corrected mass of the \Bpm meson will be denoted as $M_{\mathrm{B_{corr}}}$. \DIFdelbegin \DIFdel{The }\DIFdelend $M_{\mathrm{corr}}$ is equal to the true mass if the missing part of the decay has zero mass and has no momentum along the $\Bpm$ flight direction. Otherwise, $M_{\mathrm{corr}}$ is below the $\Bpm$ mass.
%\begin{equation}
...
\end{aligned}
\end{equation}
In this formalism $f$ is the corrected mass and $x,y,z$ are variables on which \DIFaddbegin \DIFadd{the }\DIFaddend corrected mass depends. Using~\autoref{eq:corrm}, these independent variables are visible mass four-vector, $p_{3\mu}=\{E,p_{x},p_{y},p_{z}\}$, and missing $p_{T}$ (defined in~\autoref{eq:ptmis}), which in turn depends on $\vec{p}$, $\vec{x_{PV}}$ and $\vec{x_{SV}}$.
...
\end{aligned}
\end{equation}
It was shown in~\cite{Egede:1694339} that \DIFdelbegin \DIFdel{the }\DIFdelend $\delta_\mathrm{{corr}}$ is mostly dominated by \DIFaddbegin \DIFadd{the }\DIFaddend vertex position terms \DIFaddbegin \DIFadd{$\vec{{x}}_{PV}$ and $\vec{{x}}_{SV}$}\DIFaddend .
...
\label{bkgquick}
The largest background that looks similar to signal comes from \textit{cascade decays}, where the semileptonic $b \rightarrow c \rightarrow s$ or $\bar{b} \rightarrow \bar{c} \rightarrow \bar{s}$ transitions occur. A typical example of this type of background in hadronic terms is \DIFdelbegin \DIFdel{$B^{+} \rightarrow (\bar{D}^{0} \rightarrow (K^{+} \rightarrow \mu^{-} \nu)\ \mu^{+} \nu$}\DIFdelend \DIFaddbegin \DIFadd{$B^{+} \rightarrow (\bar{D}^{0} \rightarrow (K^{+} \mu^{-} \nu)\ \mu^{+} \nu$}\DIFaddend ), where the $K^{+}$ meson is subsequently misidentified as a muon. Because the $K^{+}$ meson is misidentified as a muon, this type of background is denoted as misID background.
...
%%
%%
%DIF < %In~\autoref{fitsens}, two fitting strategies for the signal data are described. One is more sensitive than another, where the fitting strategy that provides the best sensitivity makes use of simultaneous fit to two bins of resolution, increasing signal separation from background. More information about this split can be found in~\autoref{split}. Both fitting strategies were applied on blinded signal datasets in order to compute the expected exclusion limit on the branching fraction, shown in~\autoref{sensitivity}. Systematic uncertainties that are affecting this measurement are presented in~\autoref{systematics}.
%DIF > %In~\autoref{fitsens}, two fitting strategies for the signal data are described. One is more sensitive than the other, where the fitting strategy that provides the best sensitivity makes use of simultaneous fits to two bins of resolution, increasing signal separation from background. More information about this split can be found in~\autoref{split}. Both fitting strategies were applied on blinded signal datasets in order to compute the expected exclusion limit on the branching fraction, shown in~\autoref{sensitivity}. Systematic uncertainties that affect this measurement are presented in~\autoref{systematics}.
%%
%%
%%Upon unblinding, no significant signal was observed and a stringent limit $\mathcal{B}(\Bmumumu) < 1.4\times 10^{-8}$ at $95\%$ confidence level was set using the $CL_{s}$ method with a simultaneous fit. All information about the result as well as the implication can be found in~\autoref{chap:Results}.% This result does not confirm the prediction for the decay branching fraction\cite{Danilina:2017bcn}\cite{Danilina:2018uzr}, where $\mathcal{B}(\Bmumumu) \approx 1.3\times 10^{-7}$.
%%
...
The \textit{INSP} model is used as the default model for mass fit shapes and efficiency calculations. \DIFdelbegin \DIFdel{The }\DIFdelend \textit{PHSP} \DIFaddbegin \DIFadd{decay model }\DIFaddend is used for alternative efficiency calculations. Finally \DIFaddbegin \DIFadd{the }\DIFaddend \textit{NIKI} model is used for validation purposes.
...
Firstly, all three muon tracks are required to have a significant \gls{IP} with respect to the primary vertex. Minimum Impact Parameter $\chi^{2}$, \gls{minipchi2}, gives the minimum \DIFdelbegin \DIFdel{significance }\DIFdelend \DIFaddbegin \DIFadd{$\chi^{2}$ distance }\DIFaddend of a particle's trajectory to \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{any }\DIFaddend primary vertex. Hence \DIFdelbegin \DIFdel{by }\DIFdelend requiring \gls{minipchi2}$>9$ for muons is consistent with the hypothesis that the muon is $3\sigma$ away from \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{any }\DIFaddend primary vertex and hence can be well differentiated. In addition, the change in the $\chisq$ if \DIFaddbegin \DIFadd{the }\DIFaddend \gls{PV} and \gls{SV} vertices are fitted separately as opposed to \DIFaddbegin \DIFadd{a }\DIFaddend common vertex fit, \gls{fdchi2}, suppresses prompt backgrounds. 

Each muon track is required to have good \DIFdelbegin \DIFdel{track $\chi^{2}$ per number of degrees of freedom of the fit, }\DIFdelend \gls{trackchi2ndof}, as well as low \gls{pgh2}. This removes spurious tracks as well as tracks with low quality.

Each muon candidate is also identified with initial basic \gls{PID} variables. Firstly muons are chosen due to their signature in the \DIFdelbegin \DIFdel{muons }\DIFdelend \DIFaddbegin \DIFadd{muon }\DIFaddend stations with the binary \texttt{isMuon} decision. Secondly, muon \DIFdelbegin \DIFdel{candidates are }\DIFdelend \DIFaddbegin \DIFadd{candidate is }\DIFaddend chosen such that it is more likely that the candidate is a muon than a pion or kaon using global DLLmu variables defined in~\autoref{muonID}. This reduces the background from misidentified muons.

In order to only select events which are compatible with the three muons originating from the same point in \DIFdelbegin \DIFdel{the }\DIFdelend space, \gls{vertexchi2ndof}, the $\chi^{2}$ of the trimuon vertex per degree of freedom fit is required to be small. This decreases the contamination from \textit{cascade decays} where the particle with the $c$ quark content from $b \rightarrow \underline{c} \rightarrow s$, such as $D$, would have non-negligible lifetime leading to higher \gls{vertexchi2ndof}. 

A requirement that \Bp direction points in the same direction as the line from \gls{PV} to \gls{SV} is done by making sure that \gls{DIRA}, angle between the two vectors, is close to unity. This translates into a well reconstructed event, which minimizes combinatorial background, where \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend random track makes this pointing worse. Putting bounds on the mass window, whether it is \textit{visible} or \textit{corrected} mass \DIFaddbegin \DIFadd{(defined in~}\autoref{eq:corrm}\DIFadd{)}\DIFaddend , also suppresses combinatorial events. % This is because of on average higher momentum combinatorial muon.  
...
%DIF > 	muon & $p_{T} >$ 0 \\
	muon & \DIFdelbeginFL \DIFdelFL{$p_{T} >$ 0 }%DIFDELCMD < \\
%DIFDELCMD < 	%%%
\DIFdelFL{muon }%DIFDELCMD < & %%%
\DIFdelendFL \gls{trackchi2ndof}$ < 3$ \\
...
\end{center}
	\caption{Selection of events based on \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL muon and the $B^{+}$ candidate requirements. \textit{Stripping selection} for the signal decay $B^{+} \rightarrow \mu^{+} \mu^{-} \mu^{+} \nu_\mu$ is the same for both Run \Rn{1} and 2016 data.}
\label{tab:stripcutsB}
\end{table}

\section{Trigger Selection}
In order to obtain triggered data, $B^{+} \rightarrow \mu^{+} \mu^{-} \mu^{+} \nu_\mu$ candidates are required to pass a certain set of trigger decisions at \gls{L0}, \gls{HLT1} and \gls{HLT2} as summarized in~\autoref{tab:triggersel}. It can be noted that the decision is applied at the mother \Bpm level. In particular, \DIFaddbegin \DIFadd{a }\DIFaddend positive \texttt{Bplus$\_$L0MuonDecision$\_$TOS} decision means that one of the muons from \DIFaddbegin \DIFadd{the }\DIFaddend \Bpm in an event triggered $\texttt{L0Muon}$.
...
As discussed in~\autoref{triggerchap} \texttt{L0MuonDecision} decides on whether an event is accepted depending on the $p_{T}$ of a muon and the number of hits in the \gls{SPD}. Run \Rn{1} can be split into 2011 and 2012 conditions where, in 2011 the most used \DIFdelbegin \DIFdel{threshold }\DIFdelend \DIFaddbegin \DIFadd{thresholds }\DIFaddend for positive decision \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend 1.48 \gevc \cite{Aaij:2012me} and 1.76 \gevc \cite{Albrecht:2013fba}. Run \Rn{1} \gls{SPD} rate only accepts events below \DIFdelbegin \DIFdel{600. }\DIFdelend \DIFaddbegin \DIFadd{600 hits. }\DIFaddend In 2016, the trigger thresholds varied more but the most representative acceptance for muon $p_{T}$ was above 1.85 \gevc with \gls{SPD} multiplicity below \DIFdelbegin \DIFdel{450.
}\DIFdelend \DIFaddbegin \DIFadd{450 hits.
}\DIFaddend 

\texttt{Hlt1TrackMuonDecision} accepts events where at least one identified muon has to pass thresholds on \gls{ipchi2}, $p_{T}$ and $p$. This favours muons arising from $b$- and $c$-hadron decays. There has to be at least one muon (\texttt{isMuon==true}) \DIFdelbegin \DIFdel{in its final state }\DIFdelend with certain kinematic thresholds on $p$ and $p_{T}$. For example, in 2011 the identified muons that triggered \DIFaddbegin \DIFadd{a }\DIFaddend positive decision had to have $p$ above 8 \gevc \cite{Aaij:2012me}.

At \gls{HLT2} level, the candidates are required to pass through at least one of the four \DIFdelbegin \DIFdel{decisions. }\DIFdelend \DIFaddbegin \DIFadd{following decisions. The }\DIFaddend \texttt{Hlt2TopoMu[2,3]BodyBBDTDecision} \DIFdelbegin \DIFdel{belong }\DIFdelend \DIFaddbegin \DIFadd{belongs }\DIFaddend to the \textit{topological triggers} category with an extra requirement of a particle \DIFdelbegin \DIFdel{in a candidate }\DIFdelend being identified by the \texttt{isMuon} decision. \DIFaddbegin \DIFadd{The }\DIFaddend \texttt{Hlt2DiMuonDetachedDecision} and \texttt{Hlt2DiMuonDetachedHeavyDecision} reconstruct decays with two muons in the final state. The two lines differ in that they are optimised for heavy and light dimuon pairs respectively. For example, \texttt{Hlt2DiMuonDetachedDecision} accepts events with \DIFaddbegin \DIFadd{a }\DIFaddend dimuon $p_{T}$ above 1.5 \gevc and with mass above 1 \gevcc, whereas  \texttt{Hlt2DiMuonDetachedHeavyDecision} accepts dimuon pairs with any $p_{T}$ but above 2.95 \gevcc in mass. The reason why these lines are called detached are because individual muons are required to have high \gls{ipchi2}.
...
From the two invariant mass squared pairs one can define, $minq^2 = min[q^{2}(\mu_1,\mu_2), q^2(\mu_2,\mu_3)]$ and $maxq^{2} = max[q^{2}(\mu_1,\mu_2), q^2(\mu_2,\mu_3)]$. This measurement is made in \DIFaddbegin \DIFadd{a }\DIFaddend region where $minq=\sqrt{minq^{2}}<980$ \mevcc for \DIFdelbegin \DIFdel{two }\DIFdelend \DIFaddbegin \DIFadd{few }\DIFaddend reasons: most of the contributions to the amplitude of the decay \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend below this value\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend combinatorial background is greatly reduced if $minq<1\gevcc$, see~\autoref{fig:qsqsel}. \DIFaddbegin \DIFadd{The choice of $980 \mevcc$ was made in order to avoid possible backgrounds from $\phi\rightarrow \mu^{+}\mu^{-}$.
}\DIFaddend 
...
\end{tabular}
\end{center}
\caption{\DIFdelbeginFL \DIFdelFL{BDT variables }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Variables }\DIFaddendFL used in both \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL combinatorial and misID \DIFaddbeginFL \DIFaddFL{BDTs in }\DIFaddendFL Run \Rn{1} and 2016 BDTs.}
\label{tab:vars}
\end{table}
...
The isolation quality of the vertex is determined with a BDT. This regression algorithm classifies the event to be more signal-like or background-like according to different track and vertex properties, the \textit{isolation variables}. The \textit{isolation variables} include track $p_T$, the opening angle between a track's momentum and momentum of the combined \DIFdelbegin \DIFdel{signal/background }\DIFdelend visible system, the \gls{trackchi2ndof}, the ghost probability of the track \gls{pgh2}, \gls{ipchi2} of the track with respect to \DIFdelbegin \DIFdel{signal/background }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend \gls{SV} and \gls{PV}.
...
\label{CombiBDTsel}
One of the most prominent backgrounds is \DIFaddbegin \DIFadd{the }\DIFaddend combinatorial background and to reduce its contamination while keeping the signal efficiency as high as possible, a combinatorial BDT is trained.
To obtain the combinatorial BDT discriminant, a simulated sample for signal and the upper mass sideband data sample ($M_{\mathrm{B_{corr}}}>5.5$ \gevcc) for background are used. These samples passed through the preselection, trigger, $q^{2}$ selection stages\DIFdelbegin \DIFdel{and are using the }\DIFdelend \DIFaddbegin \DIFadd{. The }\DIFaddend input variables mentioned in~\autoref{tab:vars} \DIFaddbegin \DIFadd{are used}\DIFaddend .
...
The \texttt{FOM} is computed in the blinded mass region, $4.5\gevcc <M_{\mathrm{B_{corr}}} <5.5$ \gevcc \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{as this region is most sensitive to the signal. From~}\autoref{eq:punzifom} \DIFadd{it is necessary to compute the signal efficiency and number of background candidates. }\DIFaddend To estimate the number of background candidates in the blinded region, the final fit strategy described in~\autoref{fitsens} is used to fit the data\DIFdelbegin \DIFdel{, yielding }\DIFdelend \DIFaddbegin \DIFadd{. This yields }\DIFaddend around $10000$ \DIFdelbegin \DIFdel{in Run }%DIFDELCMD < \Rn{1}%%%
\DIFdel{, }\DIFdelend and $9000$ combinatorial candidates in \DIFaddbegin \DIFadd{Run }\Rn{1} \DIFadd{and }\DIFaddend 2016. The yields are extracted from blinded fits to data by integrating the combinatorial part of the total background PDF in the blinded region.
...
In both Combinatorial BDTs, the most discriminating variables are the isolation variable (described in~\autoref{isolationvar}), $B^{+}$ \gls{vertexchi2ndof}, \gls{minipchi2} of the muons and $p_{T}$ of the $B^{+}$ meson. \DIFaddbegin \DIFadd{A }\DIFaddend Combinatorial muon comes more from somewhere else in the event and hence \DIFdelbegin \DIFdel{its }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend \gls{minipchi2} is worse as compared to the signal, making the $B^{+}$ \gls{vertexchi2ndof} worse. Moreover, as this combinatorial muon comes from somewhere else, other tracks may accompany it making the isolation variable a good discriminant. The combinatorial muon also tends to have higher momentum and hence $p_{T}$ of the $B^{+}$ is higher. Distributions for these different variables can be seen in~\autoref{fig:discombi}. 
...
\centering
	...
	\caption{The variables with the most discriminative power for both Run \Rn{1} and 2016 Combinatorial BDTs. In these plots $\mu^{1}$ is one of the \DIFdelbeginFL \DIFdelFL{muon }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{muons }\DIFaddendFL with \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL charge that agrees with \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL mother $B$, so if the mother is $B^{+}$ then $\mu^{1}$ is one of the positively charged \DIFdelbeginFL \DIFdelFL{muon }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{muons }\DIFaddendFL ($\mu^{+}$).}
\label{fig:discombi}
\end{figure}
...
\centering
	...
	\caption{Study of linear correlation between BDT output and $M_{\mathrm{B_{corr}}}$ and BDT value for each bin of $M_{\mathrm{B_{corr}}}$ in 2016 shows that the Combinatorial BDT is relatively flat as a function $M_{\mathrm{B_{corr}}}$. The right plot shows the mean and uncertainty on the mean of the 2016 Combinatorial BDT in bins of the $M_{\mathrm{B_{corr}}}$. \DIFaddbeginFL \DIFaddFL{The full correlation matrix with all other variables is listed in~}\autoref{fig:correlationcombi}\DIFaddFL{.}\DIFaddendFL }%\mybox{Sally: Error do rms instead of the standard error}.}
...
\includegraphics[width=0.9\linewidth]{./sel/true_Misid_CompareTrainOnBothRun1and2016__COMPAREroccurvesDIFFERENTtrainings.pdf}
	\caption{Comparison of separate and combined training samples and performance on different datasets. The optimal working point is chosen, see~\autoref{fig:punzifommisid} and its corresponding signal efficiency in Run \Rn{1} is 0.44 and for 2016 0.37 denoted with a violet line. As the performance is better for 2016 when the training is performed separately, the training is kept \DIFdelbeginFL \DIFdelFL{separately }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{separate }\DIFaddendFL also to be consistent with \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL previous methodology.}
%\vspace*{-1.0cm}
\label{fig:separatetrainingmis}
\end{figure}

The optimisation metric for this classifier was again \DIFaddbegin \DIFadd{the }\DIFaddend Punzi \texttt{FOM} in a blinded region. The Punzi \texttt{FOM} for Run \Rn{1} and 2016 as a function of BDT cut can be seen in~\autoref{fig:punzifommisid} for both significances of $n=\{3,5\}$.
...
The most powerful variables that distinguish the signal from misID background are the kinematic properties of the misidentified muon, namely $p_{T}$, $p$ and \gls{minipchi2}. Misidentified muons tend to be softer than for the signal as they come from cascades via $D^{0}$ decays and its excited states. The \gls{minipchi2} distribution is also different as the misidentified muon can proceed from $D^{0}$ decays, whereas \DIFaddbegin \DIFadd{the }\DIFaddend signal muon comes directly from the $B$ meson. The kinematic distributions are also different for the two real muons between signal and misID background samples. The real muon that has the same charge as \DIFaddbegin \DIFadd{the }\DIFaddend $B$ meson tends to be softer for the signal case whereas the real muon that has opposite charge proceeding via the $D$ meson will be harder, as seen in~\autoref{fig:dismisif}.
...
\centering
	...
	\caption{The variables with the most discriminative power for both misid Run \Rn{1} and 2016 \DIFdelbeginFL \DIFdelFL{BDT}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{BDTs}\DIFaddendFL . In these plots, for signal samples $\mu^{1}$ and $\mu^{3}$ are the muons with the charge that agrees with \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL mother $B$, so if the mother is $B^{+}$ then $\mu^{1}$ and $\mu^{3}$ are the positively charged muons ($\mu^{+}$), and $\mu^{2}$ is negatively charged muon ($\mu^{-}$). For misID data samples, $\mu^{1}$ is \DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL true muon with the charge that agrees with the charge of \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL mother $B$, $\mu^{2}$ is \DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL true muon with the charge that does not agree with the charge \DIFaddbeginFL \DIFaddFL{of the }\DIFaddendFL mother $B$ and finally $\mu^{3}$ is the misidentified particle whose charge agrees with the charge of \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL mother $B$.}
\label{fig:dismisif}
\end{figure}
...
Because the signal mass distribution is expected to be in a more narrow window around the $B^{+}$ peak in corrected mass and the exponential description of \DIFaddbegin \DIFadd{the }\DIFaddend combinatorial background is not correct below $4000$ \mevcc, as will be shown in~\autoref{combiback}, the \DIFdelbegin \DIFdel{fitting region selection }\DIFdelend \DIFaddbegin \DIFadd{region }\DIFaddend in which the measurement will be made is \DIFaddbegin \DIFadd{selected as }\DIFaddend $4000$ \mevcc $<M_{\mathrm{B_{corr}}}<$ $7000$ \mevcc.
...
\DIFaddbegin \DIFadd{This is the final step in selection.
}\DIFaddend 

\DIFaddbegin \DIFadd{The overall selection results in 1198 candidates when signal region is blinded. There are no events with multiple candidates. The total efficiency for selecting the signal is about 0.1\%. The blinded data can be seen in~}\autoref{fig:blsel}\DIFadd{. 
}


\begin{figure}[ht]
\centering
\includegraphics[width=0.7\linewidth]{./sel/justdate2.pdf}
	\caption{\DIFaddFL{The blinded dataset after full selection.}}
\label{fig:blsel}
\end{figure}


\DIFaddend %\section{Multiple Candidates}
...
\end{equation}
where $\delta_\mathrm{{corr}}$ is the estimate of the corrected mass error. Because the corrected mass error has \DIFaddbegin \DIFadd{a }\DIFaddend clear dependence on resolution (see ~\autoref{fig:resolution}), this split will divide the data into two bins of resolution increasing the sensitivity for observation as shown in~\autoref{fig:resofit}.
\begin{figure}[H]
\centering
...
The split boundary was chosen in such a way as to keep $\sim50\%$ of \DIFaddbegin \DIFadd{the }\DIFaddend signal in $\sigma_{\rm{lowFCME}}$ and $\sim50\%$ \DIFaddbegin \DIFadd{of the }\DIFaddend signal in high $\sigma_{\rm{highFCME}}$.
