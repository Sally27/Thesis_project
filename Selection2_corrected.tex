...
Throughout the analysis there was a blinding procedure put in place for this search in order not to bias the result. The \DIFaddbegin \DIFadd{corrected mass of the $B^{\pm}$ ($M_{\rm{B_{corr}}}$), detailed in~}\autoref{eq:corrm}\DIFadd{, is the fit variable for this search. The }\DIFaddend signal region $4500{\mevcc}<M_{\rm{B_{corr}}}<{5500\mevcc}$ was blinded until the full strategy and sensitivity was evaluated. The signal dataset corresponding to this selection is known as \textbf{blinded signal dataset}. After unblinding, data with the full mass spectrum was obtained and is known as \textbf{full signal dataset}.%  Hence in all blinded fits only data in regions $4000{\mevcc}<M_{\rm{B_{corr}}}<{4500\mevcc}$ and $5500{\mevcc}<M_{\rm{B_{corr}}}<{7000\mevcc}$ are used.
...
As discussed in~\autoref{triggerchap} \texttt{L0MuonDecision} decides on whether an event is accepted depending on the $p_{T}$ of a muon and the number of hits in the \gls{SPD}. Run \Rn{1} can be split into 2011 and 2012 conditions where, in 2011 the most used \DIFdelbegin \DIFdel{threshold for positive decision is }\DIFdelend \DIFaddbegin \DIFadd{thresholds for positive decisions are }\DIFaddend 1.48 \gevc \cite{Aaij:2012me} and 1.76 \gevc \cite{Albrecht:2013fba}. Run \Rn{1} \gls{SPD} rate only accepts events below \DIFdelbegin \DIFdel{600. }\DIFdelend \DIFaddbegin \DIFadd{600 hits. }\DIFaddend In 2016, the trigger thresholds varied more but the most representative acceptance for muon $p_{T}$ was above 1.85 \gevc with \gls{SPD} multiplicity below \DIFdelbegin \DIFdel{450.
...
\DIFaddbegin \DIFadd{The overall selection results in 1198 candidates when the signal region is blinded. There are no events with multiple candidates. The total efficiency for selecting the signal is about 0.1\%. The blinded data can be seen in~}\autoref{fig:blsel}\DIFadd{. 
}
...
Secondly, \DIFaddbegin \DIFadd{all the offline signal selection cuts listed in~}\autoref{tab:OfflineSelection} \DIFadd{are also applied apart from a few exceptions. As }\DIFaddend this decay proceeds via the $J/\psi$ resonance and hence the $q^{2}$ veto for $J/\psi$ and $\Psi{(2S)}$, listed in~\autoref{tab:vetoes}, is not applied but rather reversed as seen in~\autoref{tab:stripcutsBnorm}. As the third particle is a kaon rather than a muon, there is no explicit choice of $minq$ region.
