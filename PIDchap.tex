...
In order to obtain $K/\pi$ misid probabilities a fit to $\mu^{+} \mu^{-} \pi^{+} K^{-}$ mass between 5150 - 5450 \mevcc was performed. This fit with free parameters \mybox{say exactly which parameters}, fits that data and determines the yield of all the components. 


The actual determination of the misid rate was obtained using a statistical subtraction of background, known as the \textit{sPlot} technique \mybox{reference}. The same is used in the \texttt{PIDCalib} package. \mybox{say something about splot}. 
...
\subsection{Results of  $B^{0} \rightarrow J/\psi(\rightarrow \mu^{+} \mu^{-}) K^{*}$ control sample for \mb{$K/\pi \rightarrow \mu$} misid rates \mybox{ULRIK}}
