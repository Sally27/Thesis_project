...
In order to establish species of particles for each track, the Cerenkov angle is combined with the track momentum measured by tracking. In practice, however, as RICH detectors operate in high track density environment, many Cerenkov rings will be overlapping and hence a complex pattern recognition algorithm is deployed. For each event, all the tracks in both RICH detectors are assigned all possible mass hypotheses to give a full event likelihood \cite{Forty:1999sg}.   
